...
class CustomScanNotifier extends _$CustomScanNotifier {
  ...
  void selectFilter(ScanFilter filter) {
  ...
  Future<void> onRawCaptured(String path) async {
  ...
  Future<void> confirmCrop() async {
    ...
    state = state.copyWith(busy: true, busyMessage: 'Cropping…', clearError: true);
    try {
      final String warped = await ref.read(edgeDetectServiceProvider).warp(
            imagePath: path,
            quad: quad,
          );
      state = state.copyWith(
  ...
  Future<void> confirmEnhance() async {
    final String? warped = state.warpedPath;
    if (warped == null) {
      return;
    }
    state = state.copyWith(busy: true, busyMessage: 'Applying filter…', clearError: true);
    try {
      final String filtered = await ref.read(scanEnhanceServiceProvider).applyFilter(
            imagePath: warped,
  ...
  Future<void> _saveDocument() async {
    final List<String> paths =
        state.pages.map((ScanPageDraft p) => p.imagePath).toList();
